...
namespace meow {
	...
	inline Matrix33 meow::Matrix33::CreateTranslation(const vec2& translate)
	{
	...
	inline vec2 meow::Matrix33::GetTranslation() const
	{
		return { rows[0][2] , rows[1][2] };
	}
	inline float meow::Matrix33::GetRotation() const
	{
		return std::atan2(rows[1][0], rows[0][0]);
	}
	inline vec2 meow::Matrix33::GetScale() const
	{
